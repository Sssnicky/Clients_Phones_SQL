...
with p.connect(database="clients_db", user="postgres", password="postgres") as conn:
    create_db(conn)
    add_client(conn, 'Casey', 'Neistat', 'neistat@gmail.com', ['+442345676', '+445678899'])
    add_client(conn, 'Marques', 'Brownlee', 'mkbhd@gmail.com', ['+13456677'])
    add_client(conn, 'Elon', 'Musk', 'musk@yahoo.com', ['+14556677'])
    select_all(conn)
    print('\n')
    change_client(conn, 3, 0, 'Notmusk')
    find_client(conn, 'Elon')
    select_all(conn)
    print('\n')
    delete_phone(conn, '442345676')
